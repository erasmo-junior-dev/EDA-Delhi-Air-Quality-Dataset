...
AS VARIÁVEIS: Month, Holidays_Count, Days, Date, Month, Year são todas categóricas apesar de serem vistas como números
...
df$Holidays_Count <- as.factor(df$Holidays_Count)
```
...
df$Holidays_Count <- as.numeric(as.character(df$Holidays_Count))
...
Algumas relações claras podem ser vistas, como por exemplo de particulas finas (PM2.5 e PM10) com a qualidade do ar, quanto mais particulas pior é a qualidade, assim como monóxido de carbono (CO). Também é perceptível que a concentração de ozônio diminui a concentração de outros elementos.
...
- Correlação
```{r, correlaction PM2.5 and AQI}
cor(df$PM2.5, df$AQI)
```

- Modelo Linear
```{r, linear model}
linear_model <- lm(AQI ~ PM2.5, data = df)
...
```{r, intercession}
coef_intercepto <- round(coef(linear_model)[1], 2)
```

- Inclinação
```{r, inclination}
coef_inclinacao <- round(coef(linear_model)[2], 2)
```

- R²
```{r, r²}
r2 <- round(sum_linear_model$r.squared, 3)
r2
```

- Equação da Reta
```{r, equation of a line}
equacao <- paste0("y = ", coef_intercepto, " + ", coef_inclinacao, "x")
equacao
```

- Gráfico de Dispersão e Linha de Tendência
```{r, dispersion plot and tendency line}
plot(df$PM2.5, df$AQI, main="Concentração de PM2.5 X AQI", col="darkblue",
     ylab="Nível de AQI", xlab="Nível de Concenctração de PM2.5 (µg/m³)")

text(750, 80 + offset, labels = equacao, col = "black", cex = 1.1)
text(750, 40, labels = paste0("R² = ", r2), col = "black", cex = 1.1)
