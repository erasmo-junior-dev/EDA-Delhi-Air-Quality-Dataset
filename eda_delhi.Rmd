...
print(sum_linear_model)
```

- Ponto de Intercessão
```{r, intercession}
intercession <- coef(linear_model)[1]
intercession
```

- Inclinação
```{r, inclination}
inclination <- coef(linear_model)[2]
inclination
```
...
equacao <- paste0("y = ", round(intercession, 2), " + ", round(inclination, 2), "x")
equacao
```
...
text(4, 80 + offset, labels = equacao, col = "black", cex = 1.1)
text(4, 40, labels = paste0("R² = ", r2), col = "black", cex = 1.1)
```
